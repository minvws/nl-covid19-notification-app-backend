...
describe('Stopkeys endpoints tests #stopkeys #endpoints #regression', async function() {
    ...
    before(function (){
        function stopKeys (){
            return new Promise(function (resolve){
                ...
                Promise.all([register, sig]).then(function([resultRegister, resultSig]) {
                    // more processing
                    app_post_postkeys_controller.postkeys(env.STOPKEYS,padding,resultRegister.bucketId,resultSig.sig).then(function (response){
        ...
        return stopKeys().then(function (response){
            apiResponse = response;
            content = apiResponse.data
        });
    });

    it('A stopkeys response structure validation', function (){
    ...
    it('A  stopkeys response is empty', function (){
        expect(content).to.be.empty;
    })

    it('Stopkeys response time is under 200 ms.', function (){
