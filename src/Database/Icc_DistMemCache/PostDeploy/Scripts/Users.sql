...
IF NOT EXISTS (SELECT name FROM sys.server_principals WHERE name = N'$(Domain)\$(Portal)')
CREATE LOGIN [$(Domain)\$(Portal)] FROM WINDOWS;
...
IF NOT EXISTS (SELECT name FROM [sys].[database_principals] WHERE name = N'$(Domain)\$(Portal)')
CREATE USER [$(Domain)\$(Portal)];