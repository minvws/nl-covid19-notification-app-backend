...
write-host "Certificate renewal-script for interop signing-certificate with Utimaco HSM"
write-host "Location and date: $env:computername. $(Get-Date -Format `"dd MMM, HH:mm:ss`")."
CheckNotIse
CheckNotWin7

write-warning "`nPlease check the following:`
- This script renews only the signing-certificate!
...
if($IsOnDevEnvironment -eq $true)
{
	...
	write-host "`nAdding Signing-certificate to local machine store"
	Pause
...
else
{
	...
	$FriendlyName = read-host "`nPlease enter a `'Friendly name`' for the certificate.`n Make sure the name is not already in use! (look inside the machine personal keystore)"
...
else
{
	...
	write-host "`nDone! The Signing certificate request is $signerName.csr.`n"
	write-warning "When submitting the signing-request, don't forget to add the 2-year validity period."
}