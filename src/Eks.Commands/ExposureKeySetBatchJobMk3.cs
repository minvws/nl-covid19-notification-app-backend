...
namespace NL.Rijksoverheid.ExposureNotification.BackEnd.EksEngine.Commands
{
    ...
    public sealed class ExposureKeySetBatchJobMk3
    {
        ...
        public async Task<EksEngineResult> ExecuteAsync()
        {
            ...
            if (_EksConfig.PublishEveryRun || snapshotResult.TekInputCount != 0)
            {
                await StuffAsync();
        ...
        private async Task StuffAsync()
        {
            ...
            // NOTE: stuffing must be generated when publishing every run even when there are no real TEKs
            if (!_EksConfig.PublishEveryRun)
            {
                if (tekCount == 0)
                {
