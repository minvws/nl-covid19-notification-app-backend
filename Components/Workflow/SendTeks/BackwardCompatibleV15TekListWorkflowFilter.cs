...
namespace NL.Rijksoverheid.ExposureNotification.BackEnd.Components.Workflow.SendTeks
{
    public class BackwardCompatibleV15TekListWorkflowFilter : ITekListWorkflowFilter
    {
        ...
        public Results<Tek> Validate(Tek[] newKeys, TekReleaseWorkflowStateEntity workflow)
        {
            ...
            _Workflow = workflow;
            _WorkflowHasKeyOnCreationDate = _Workflow.Teks.Any(x => x.RollingStartNumber.FromRollingPeriodStart().Date == _Workflow.Created.Date);
        ...
        private string[] ValidateSingleTek(Tek item)
        {
            ...
            if (item.StartDate == _Now.Date)
            {
                ...
                if (_Workflow.AuthorisedByCaregiver != null && !((_Now - _Workflow.AuthorisedByCaregiver) < TimeSpan.FromMinutes(120))) //TODO setting
